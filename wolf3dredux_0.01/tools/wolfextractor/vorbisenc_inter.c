...
#include "vorbisenc_inter.h" /* new location for prototypes from this file */
...
/* prototype moved to "vorbisenc_inter.h" */
long read_samples(float **buffer, int samples)
{
	...
	sampbyte = (int)(samplesize / 8); /* initialize separately */

	buf = (SW8*)ptrCurrent;

	if (((unsigned long)(samples * sampbyte) * channels) >
		(unsigned long)(ptrEnd - ptrCurrent)) {
		...
		bytes_read = (long)((unsigned long)(samples * sampbyte) * channels);
		ptrCurrent += ((unsigned long)(samples * sampbyte) * channels);
	}

	realsamples = (long)((unsigned long)(bytes_read) /
						 ((unsigned long)(sampbyte) * channels));

	for ((i = 0); (i < realsamples); ++i) {
		for ((j = 0); ((W32)(j) < channels); ++j ) {
			/* this whole next assignment is scary: */
			buffer[j][i] = ((unsigned long)(buf[(((unsigned long)(i * 2) * channels) +
												 (unsigned long)(2 * j) + 1)] << 8) |
							(unsigned long)((buf[(((unsigned long)(i * 2) * channels) +
												  (unsigned long)(2 * j) + 0)] & 0xFF) / 32768.0f));
		}
	}
...
/* prototype moved to "vorbisenc_inter.h" */
int vorbis_encode(const char *filename, void *data, W32 size, W32 in_channels,
				  W32 in_samplesize, W32 rate, W32 quality, W32 max_bitrate,
				  W32 min_bitrate)
{
	FILE *fp;
	ogg_stream_state os;
	...
	if ((max_bitrate > 0) || (min_bitrate > 0)) {
		...
		ai.bitrate_hard_min = (long)min_bitrate;
		ai.bitrate_hard_max = (long)max_bitrate;
	...
	ogg_stream_init(&os, (int)serialno);
	...
	while (! eos) {
		...
		if (samples_read == 0) {
			...
			samplesdone += (unsigned long)samples_read;
		...
		while (vorbis_analysis_blockout(&vd, &vb) == 1) {
			...
			while (vorbis_bitrate_flushpacket(&vd, &op)) {
				...
				/* If we have gone over a page boundary, we can do actual output,
				 * so do so (for however many pages are available) */

				while (! eos) {
					/* 'result' is already declared as an 'int', so we can just
					 * assign to it here, without having to re-declare: */
					...
					if (ret != og.header_len + og.body_len) {
						...
						bytes_written += (unsigned long)ret;
					}
