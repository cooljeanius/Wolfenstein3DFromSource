...
PUBLIC size_t cs_strlcat(char *dest, const char *source, size_t nMaxLength)
{
	...
	/* Find the end of dest and adjust bytes left but do NOT go past end */
	while ((n-- != 0) && (*d != '\0')) {
		d++;
	}
...
{
	W32 hash;
	hash = (W32)(*string); /* initialize separately */

	if (hash) {
		for ((string += 1); (*string != '\0'); ++string) {
			hash = ((hash << 5) - hash + (unsigned long)(*string));
			/* really, that is all that it does? */
		}
	}
...
/*
 -----------------------------------------------------------------------------
 Function: cs_CopyString - copies a string, I guess?

 Parameters: string -[in] NUL-terminated string to be... copied, I guess?.

 Returns: Another string?

 Notes: I did not know what exactly the original author had in mind when they
		put a prototype for this function in "com_string.h", or when they used
		it in "../../../common/cvar.c", so I am just going to try implementing
		it as a simple wrapper around 1 of either:
		stpcpy(), strcpy(), or strncpy()...
		(I have not decided which of the 3 would be best yet... strncpy() is
		more secure, but strcpy() is easier to work with...)
 EDIT:  Actually never mind, I discovered that the _other_ "com_string.c" has a
		"my_CopyString()" function that, according to the _other_ "com_string.h",
		is supposted to be used like cs_CopyString() here...
		darn duplicated code... anyway, that other function is availabe as
		reference, at least...
 -----------------------------------------------------------------------------
 */
PUBLIC char *cs_CopyString(const char *in)
{
	char *out;
#  ifndef PATH_MAX
