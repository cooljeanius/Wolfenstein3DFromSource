...
PRIVATE void Action_Draw( menuaction_s *a, int i )
{
	...
	if( a->generic.flags & MF_LEFT_JUSTIFY ) {
		if( (i == a->generic.parent->cursor) && a->generic.fontHighColour ) {
			Menu_DrawString( a->generic.fs, a->generic.x + a->generic.parent->x,
							 a->generic.y + a->generic.parent->y, a->generic.name,
							 *a->generic.fontHighColour );
		} else {
			Menu_DrawString( a->generic.fs, a->generic.x + a->generic.parent->x,
							 a->generic.y + a->generic.parent->y, a->generic.name,
							 *a->generic.fontBaseColour );
		}
	} else {
		if( (i == a->generic.parent->cursor) && a->generic.fontHighColour ) {
			Menu_DrawStringR2L( a->generic.fs, a->generic.x + a->generic.parent->x + LCOLUMN_OFFSET,
							    a->generic.y + a->generic.parent->y, a->generic.name,
							    *a->generic.fontHighColour );
		} else {
			Menu_DrawStringR2L( a->generic.fs, a->generic.x + a->generic.parent->x + LCOLUMN_OFFSET,
...
_boolean Field_Key( menufield_s *f, int key )
{
	...
	*/
	...
	switch (key) {
		...
		case K_SPACE:
		default:
			if( ! isdigit( key ) && ( f->generic.flags & MF_NUMBERSONLY ) ) {
				return false;
			}
...
void Menu_AdjustCursor( menuframework_s *m, int dir )
{
	...
	/*
	** see if it is/was in a valid spot
	*/
	...
	/*
	** it is/was not in a valid spot, so crawl in the direction indicated until
