...
dnl#
      ...
      AC_CHECK_HEADERS_ONCE([vorbis/codec.h vorbis/vorbisenc.h])
      AC_CHECK_FUNCS_ONCE([vorbis_info_init vorbis_encode_init \
                           vorbis_analysis_init vorbis_block_init \
                           vorbis_bitrate_addblock system])
...
int main ()
{
    ...
    /* this function was added in 1.0rc3, so this is what we are testing
     * for */
...
]])],[],[no_vorbis=yes],[echo ${ac_n} "cross compile; assume OK ${ac_c}"])
...
#include <vorbis/codec.h>
       ...
       echo "*** This usually means that the run-time linker is not"
       echo "*** finding Vorbis or finding the wrong version of Vorbis."
       echo "*** If it is not finding Vorbis, you will need to set your"
       echo "*** LD_LIBRARY_PATH environment variable (or equivalent),"
       echo "*** or edit /etc/ld.so.conf to point to the installed"
       echo "*** location  Also, make sure you have run ldconfig if that"
       echo "*** is required on your system."
       echo "***"
       echo "*** If you have an old version installed, it is best to"
       echo "*** remove it, although you may also be able to get things"
       echo "*** to work by modifying LD_LIBRARY_PATH (or equivalent)."
],[
       echo "*** The test program failed to compile or link."
       echo "*** See the file config.log for the exact error that occured."
       echo "*** This usually means Vorbis was incorrectly installed"
