...
{
	Cvar_Set("s_device",
			 s_sound_device_list.itemnames[s_sound_device_list.curvalue]);
}

/* is the 'unused' parameter really necessary? */
PRIVATE void UpdateMasterVolumeFunc(void *unused)
{
	Cvar_SetValue("s_masterVolume",
				  (s_sound_mastervolume_slider.curvalue / 10));
}
...
PRIVATE void Sound_MenuInit(void)
{
	...
	if (sound_initialized) {
		...
		s_sound_device_list.itemnames = (const char **)&sound_devices;
...
PRIVATE void Sound_MenuDraw(void)
{
	if (g_version->value == SPEAROFDESTINY) {
		...
		M_DrawWindow(((viddef.width - 500) >> 1),
					 (((viddef.height - 150) >> 1) + 35), 500, 150,
