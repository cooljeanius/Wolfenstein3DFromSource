...
PRIVATE void Intro_HighScore_Draw( void )
{
	...
	for( i = 0, hs_temp = Scores ; i < MAX_SCORES ; ++i, ++hs_temp ) {
		...
		/* Episode / Level */
		my_snprintf( string, sizeof( string ), "E%d/L%d", hs_temp->episode+1, hs_temp->completed );
...
PRIVATE void Intro_SHighScore_Draw( void )
{
	...
	for( i = 0, hs_temp = Scores ; i < MAX_SCORES ; ++i, ++hs_temp )
	{
		...
		/* Episode / Level */
		my_snprintf( string, sizeof( string ), "%d", hs_temp->episode+1, hs_temp->completed );
		Font_put_line( FONT0, (40*8) + ((viddef.width - 640)>>1), y, string );
...
PRIVATE void M_Intro_Draw( void )
{
	...
	if( ClientStatic.realtime >= (intro_basetime + sIntroLayer[ intro_slide ].intro_time) )
	{
