...
-----------------------------------------------------------------------------
 ...
 Notes: A simple wrapper around Z_Free() for now...
-----------------------------------------------------------------------------
*/
...
PRIVATE void FS_AddGameDirectory(const char *dir)
{
	...
	if (pakfile) {
		...
		Com_Printf("FS_AddGameDirectory(): No pakfile found here.\n");
	}


	FS_FindClose(); /* i.e. closedir() */
	...
	if (pakfile) {
		...
		Com_Printf("FS_AddGameDirectory(): No pakfile (i.e. zipfile) found here.\n");
	}
...
PUBLIC void FS_ExecAutoexec(void)
{
	...
	if (*dir) {
		my_snprintf(name, sizeof(name), "%s/%s/autoexec.cfg",
...
PUBLIC void FS_SetGamedir(char *dir)
{
	...
	if (strstr(dir, "..") || strstr(dir, "/")
		...
		Com_Printf("FS_SetGameDir(): path separator charcters found in filename:");
...
PUBLIC void FS_InitFilesystem(void)
{
	...
	if (fs_cddir->string[0]) {
		Com_Printf("FS_InitFilesystem(): Running using the following cddir/BASEDIRNAME combo: %s/%s\n",
				   fs_cddir->string, BASEDIRNAME);
	 ...
	 */
	Com_Printf("FS_InitFilesystem(): Running using the following basedir/BASEDIRNAME combo: %s/%s\n",
			   fs_basedir->string, BASEDIRNAME);
