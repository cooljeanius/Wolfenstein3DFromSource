 ...
 * each time. *his avoids any problems with getting back to a default usage
...
void IN_StartupJoystick(void)
{
	...
	/* abort startup if we did NOT find a valid joystick: */
	if (mmr != JOYERR_NOERROR) {
		Com_Printf("\njoystick not found -- no valid joysticks (%x)\n\n", mmr);
		return;
	}

	/* get the capabilities of the selected joystick, and also
	...
	/* save the number of buttons on the joystick, and the POV status: */
...
_boolean IN_ReadJoystick(void)
{
	...
	if (joyGetPosEx(joy_id, &ji) == JOYERR_NOERROR) {
		...
		/* If we are here, a read error occurred...
...
void IN_JoyMove(usercmd_t *cmd)
{
	...
	for ((i = 0); (i < JOY_MAX_AXES); i++) {
		/* get the floating-point, 0-centered, potentially-inverted data for the
		 * current axis: */
		...
		case AxisForward:
			if ((joy_advanced->value == 0.0) && mlooking) {
				...
				if (fabs(fAxisValue) > joy_pitchthreshold->value) {
					...
					/* there is only absolute control support here
			...
			if ((in_strafe.state & 1) || (lookstrafe->value && mlooking)) {
				...
				if (fabs(fAxisValue) > joy_yawthreshold->value) {
					if(dwControlMap[i] == JOY_ABSOLUTE_AXIS) {
						...
						ClientState.viewangles[YAW] += ((fAxisValue * joy_yawsensitivity->value) * speed * 180.0);
					}
			...
			if (mlooking) {
				if (fabs(fAxisValue) > joy_pitchthreshold->value) {
					...
					if (dwControlMap[i] == JOY_ABSOLUTE_AXIS) {
						...
						ClientState.viewangles[PITCH] += ((fAxisValue * joy_pitchsensitivity->value) * speed * 180.0);
					}
				}
			}
			break;
