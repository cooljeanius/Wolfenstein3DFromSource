...
PUBLIC W8 *script_ReadNumber(W8 *ptr, int numerictype, decimalType_t *nt)
{
	...
	if (numerictype == TT_FLOAT) {
		/* maybe here would be a good place to use that unused "readFloat()"
		 * function from above? */
		nt->Float = StringToFloat((const char *)buffer, &error);

		if (bNegative) {
			nt->Float = -nt->Float;
		}
	} else if (numerictype == TT_INTEGER) {
		nt->Integer = StringToInteger((const char *)buffer, &error);
...
PUBLIC _boolean script_Parse(const char *filename,
							 ...
							 W32 tagProperties_size, int tag)
{
	...
	while (*ptr != ENDOFSTREAM) {
		...
		for ((i = 0); (i < tagProperties_size); ++i ) {
			...
			if (! strncmp((const char *)ptr, tagProperties[i].name, lg)) {
				ptr += lg;

				ptr = (W8 *)tagProperties[i].parsing_function(ptr, &LineNumber);
