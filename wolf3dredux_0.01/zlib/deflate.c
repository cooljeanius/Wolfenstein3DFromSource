    ...
    int stream_size;
{
    ...
    s->window = (Bytef *)ZALLOC(strm, s->w_size, (uInt)(2 * sizeof(Byte)));
    ...
    s->d_buf = (ushf*)(overlay + (s->lit_bufsize / sizeof(ush)));
    ...
    uInt  dictLength;
{
    ...
    zmemcpy(s->window, dictionary, (size_t)length);
    s->strstart = length;
    ...
    uLong sourceLen;
{
    ...
    /* if cannot get parameters, return conservative bound */
    if (strm == Z_NULL || strm->state == Z_NULL) {
        return destLen;
	}
...
{
    unsigned len;
	len = (unsigned)(strm->state->pending);
    ...
    zmemcpy(strm->next_out, strm->state->pending_out, (size_t)len);
    strm->next_out  += len;
    ...
    int flush;
{
    ...
    if (s->pending != 0) {
        ...
        if (strm->avail_out == 0) {
             ...
             * avail_in equal to zero. There will NOT be anything to do,
    ...
    if (strm->avail_in != 0 || s->lookahead != 0 ||
        ...
        if (bstate == need_more || bstate == finish_started) {
             ...
             * that the flush is complete. So we do NOT have to output an
 ...
 * does NOT have enough memory anyway to duplicate compression states).
 */
    ...
    z_streamp source;
{
    ...
    *ds = *ss;
    ...
    ds->window = (Bytef *)ZALLOC(dest, ds->w_size, (uInt)(2 * sizeof(Byte)));
    ...
    ds->d_buf = (ushf*)(overlay + (ds->lit_bufsize / sizeof(ush)));
...
#endif /* GZIP */
    zmemcpy(buf, strm->next_in, (size_t)len);
    strm->next_in  += len;
    strm->total_in += len;
...
{
    unsigned chain_length; /* max hash chain length */
	register Bytef *scan; /* current string */
	register Bytef *match;                 /* matched string */
    register int len;                      /* length of current match */
    int best_len;                          /* best match length so far */
	int nice_match;                        /* stop if match long enough */
	IPos limit;
	Posf *prev;
	uInt wmask;
	chain_length = s->max_chain_length; /* initalize max hash chain length */
	scan = (s->window + s->strstart);  /* initialize current string */
	best_len = (int)(s->prev_length); /* initialize best match length so far */
	nice_match = s->nice_match; /* init var for stopping if match long enough */
    ...
    deflate_state *s;
{
    ...
    do {
        ...
        if (s->strstart >= (wsize + MAX_DIST(s))) {
            ...
            zmemcpy(s->window, (s->window + wsize), (size_t)(unsigned)wsize);
            s->match_start -= wsize;
             ...
             * zlib, so we do NOT care about this pathological case.)
             */
            n = s->hash_size;
            p = &s->head[n];
            do {
        ...
        n = ((unsigned int)read_buf((s->strm),
