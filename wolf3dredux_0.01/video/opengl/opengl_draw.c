...
PUBLIC void R_Draw_Character(int x, int y, int num, font_t *myfont)
{
	...
	R_Bind((int)myfont->texfont->texnum);
...
PUBLIC void R_Draw_StretchPic(int x, int y, int w, int h, const char *pic)
{
	...
	if (! gl) {
		Com_Printf("Cannot find pic: %s\n", pic);
		return;
	}
#if defined(scrap_dirty) && (defined(Scrap_Upload) || defined(HAVE_SCRAP_UPLOAD))
	if (scrap_dirty) { /* undeclared */
 		Scrap_Upload(); /* unimplemented */
	}
#endif /* scrap_dirty && (Scrap_Upload || HAVE_SCRAP_UPLOAD) */


	R_Bind((int)(gl->texnum));
...
PUBLIC void R_Draw_Tile(int x, int y, int w, int h, const char *pic)
{
	...
	R_Bind((int)(image->texnum));

	pfglBegin(GL_QUADS);


	pfglTexCoord2i((x / image->upload_width),
				   (y / image->upload_height));
	pfglVertex2i((x), (y));

	pfglTexCoord2i(((x + w) / image->upload_width),
				   (y / image->upload_height));
	pfglVertex2i((x + w), (y));

	pfglTexCoord2i(((x + w) / image->upload_width),
				   ((y + h) / image->upload_height));
...
-----------------------------------------------------------------------------
*/
PUBLIC void R_Draw_Line(int nXStart, int nYStart, int nXEnd, int nYEnd,
