...
/*
 -----------------------------------------------------------------------------
 Function: Wav_GetLittleShort

 Parameters: Nothing.

 Returns: a 'short' representing...?
...
/*
 -----------------------------------------------------------------------------
 Function: Wav_FindChunk

 Parameters: name -[in]: string to pass to Wav_FindNextChunk()

 Returns: Nothing.

 Notes: A simple wrapper around Wav_FindNextChunk() for now.
...
PRIVATE void DumpChunks(void)
{
	...
	iff_pdata = iff_data;
	do {
		memcpy(str, iff_pdata, (size_t)4);
		iff_pdata += 4;
		iff_chunk_len = Wav_GetLittleLong();
		Com_Printf("0x%x : %s (%d)\n", (int)(intptr_t)(iff_pdata - 4), str,
				   iff_chunk_len);
...
PUBLIC _boolean LoadWavInfo(const char *filename, W8 **wav, soundInfo_t *info)
{
	...
	info->channels = (W32)Wav_GetLittleShort();
	info->sample_rate = (W32)Wav_GetLittleLong();

	iff_pdata += 4;

	info->sample_size = (W32)Wav_GetLittleShort(); /* Bytes Per Sample */
	...
	iff_pdata += 4;
	info->samples = ((unsigned long)(Wav_GetLittleLong()) / info->sample_size);
	...
	*wav = Z_Malloc(info->samples * info->sample_size);
	memcpy(*wav, (data + (iff_pdata - data)),
		   (info->samples * info->sample_size));
