...
TGAReadHeader (TGA *tga)
{
	...
	/* __TGASeek is DECL-ed in tga.h, but not actually implemented? */
...
#else
		/* not sure if this works as a replacement, but we can try: */
		/* arguments to bgr2rgb are dest, src, width, bytes, alpha;
		 * as opposed to __TGAbgr2rgb, whose arguments are data, size, bytes.
		 * "width" and "size" should be equivalent, right? */
		bgr2rgb(0, 0, TGA_CMAP_SIZE(tga), tga->hdr.map_entry / 8, 0);
#endif /* __TGAbgr2rgb */
	}
				 ...
				 tuint32 flags)
{
	...
	if (TGA_CAN_SWAP(tga->hdr.depth) && (flags & TGA_RGB)) {
		 ...
		 * as opposed to __TGAbgr2rgb, whose arguments are data, size, bytes.
		 * "width" and "size" should be equivalent, right? */
		bgr2rgb(0, 0, sln_size * n, tga->hdr.depth / 8, 0);
#endif /* __TGAbgr2rgb */
	}
